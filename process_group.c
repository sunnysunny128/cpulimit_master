...
int find_process_by_name(const char *process_name)
{
	...
	{
		//process found
		if (ids_strncmp(basename(proc.command), process_name, ids_strlen(process_name))==0 && kill(pid,SIGCONT)==0) {
			//process is ok!
			pid = proc.pid;
			break;
		}
...
int init_process_group(struct process_group *pgroup, int target_pid, int include_children)
{
	...
	return update_process_group(pgroup);
}
...
int close_process_group(struct process_group *pgroup)
{
	...
	for (i=0; i<size; i++) {
		if (pgroup->proctable[i] != NULL) {
			...
			safe_free(pgroup->proctable[i]);
			pgroup->proctable[i] = NULL;
		}
	}
	clear_list(pgroup->proclist);
	safe_free(pgroup->proclist);
...
int update_process_group(struct process_group *pgroup)
{
	struct process_iterator it;
	...
	init_list(pgroup->proclist, 4);
	int ret=0;
	while((ret=get_next_process(&it, &tmp_process)) != -1)
	{
//		struct timeval t;
//		gettimeofday(&t, NULL);
